...
def gerar_excel(*, nome_fundo: str, originador: dict, estrutura, resumo: dict,
    ...
    linhas = [
        ...
        ("Prazo médio (dias)", round(estrutura.prazo_medio_meses * 30)),
        ("Revolvência (meses)", estrutura.meses_revolvencia),
        ("Rampa de integralização (meses)",
         estrutura.meses_rampa if estrutura.meses_rampa else None),
        ("Carência antes da amortização (meses)",
         estrutura.meses_carencia if estrutura.meses_carencia else None),
        ...
        ("Custo inicial one-off (R$)",
         estrutura.custo_inicial if estrutura.custo_inicial else None),
        ("Prazo máximo do fundo (meses)", estrutura.prazo_maximo_meses),
    ...
    fmt_por_item = {
        ...
        "Custo inicial one-off (R$)": FMT_RS,
